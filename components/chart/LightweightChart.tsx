...
interface ChartContext {
  api: IChartApi | null;
  isRemoved: boolean;
  free(series: any): void;
}

const Context = createContext<ChartContext>({ api: null, isRemoved: false, free: () => {} });
...
export const App = (props: any) => {
  ...
  const series1 = useRef<any>(undefined)
...
export function Chart(props: any) {
  ...
  const handleRef = useCallback(
    (ref: HTMLDivElement | null) => setContainer(!!ref),
    []
  )
  return (
    <div ref={handleRef}>
...
export const Series = forwardRef<unknown, SeriesProps>((props, ref) => {
  ...
  const context = useRef({
      ...
      if (!this?.api) {
        ...
        this.api =
          type === "line"
            ...
            : parent.api?.addAreaSeries({ options: rest })
    ...
    free() {
      ...
      if (this?.api !== undefined && !parent.isRemoved) {
        // remove only current series
        parent.free(this.api)
      }
    },
  })
